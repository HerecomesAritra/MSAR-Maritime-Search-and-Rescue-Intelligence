...
import { a as objectType, n as arrayType, o as stringType, r as enumType, t as anyType } from "../_libs/zod.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/copilot.server-CCflKBhC.js
var CopilotExplanationSchema = objectType({
	headline: stringType(),
...
var runCopilotOnServer_createServerFn_handler = createServerRpc({
	id: "a5c81c9f55c2de11d424e7905fd5c9cb48eb5e4128a9eabe2da482570eb31efb",
	name: "runCopilotOnServer",
	filename: "src/agents/copilot.server.ts"
}, (opts) => runCopilotOnServer.__executeServer(opts));
...
})).handler(runCopilotOnServer_createServerFn_handler, async ({ data }) => {
	const provider = data.provider ?? "GEMINI";
	if (provider === "OLLAMA") return runOllamaCopilot(data);
	else if (provider === "OPENROUTER") return runOpenRouterCopilot(data);
	else if (provider === "GROQ") return runGroqCopilot(data);
	else if (provider === "MISTRAL") return runMistralCopilot(data);
	else return runGeminiCopilot(data);
});
async function runGeminiCopilot(input) {
	const apiKey = input.geminiApiKey?.trim() || process.env["GEMINI_API_KEY"] || process.env["VITE_GEMINI_API_KEY"] || "";
	if (!apiKey) return parseCopilotFallback(input);
...
`;
	for (const modelName of [
		...
		const parsedData = coerceExplanationFieldsToStrings(cleanJsonResponse((await ai.models.generateContent({
			model: modelName,
			contents: prompt,
			config: {
...
function stringifyField(value) {
	...
	if (typeof value === "object") return Object.values(value).map(stringifyField).filter(Boolean).join("\n");
	return String(value);
}
function coerceExplanationFieldsToStrings(parsed) {
	if (!parsed || typeof parsed !== "object") return parsed;
	const out = { ...parsed };
	for (const field of EXPLANATION_STRING_FIELDS) if (field in out && typeof out[field] !== "string") out[field] = stringifyField(out[field]);
	return out;
}
...
async function runMistralCopilot(input) {
...
Example of a correctly-shaped value: "headline": "MRV-1 tasked to Zone-B due to fastest ETA and low sea state."`
		...
		const parsedData = coerceExplanationFieldsToStrings(cleanJsonResponse((await response.json()).choices?.[0]?.message?.content ?? ""));
...
async function runGroqCopilot(input) {
...
async function runOpenRouterCopilot(input) {
...
async function runOllamaCopilot(input) {
...
Example of a correctly-shaped value: "headline": "MRV-1 tasked to Zone-B due to fastest ETA and low sea state."`
		...
		const parsedData = coerceExplanationFieldsToStrings(cleanJsonResponse((await response.json()).message?.content ?? ""));
...
//#endregion
export { runCopilotOnServer_createServerFn_handler };
